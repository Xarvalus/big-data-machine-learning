...
/**
  * This provides an event sourced behavior. It has a state, [[ReactivestockState]], which
  ...
  */
object ReactivestockBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[ReactivestockCommand]): Behavior[ReactivestockCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, ReactivestockEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[ReactivestockCommand, ReactivestockEvent, ReactivestockState](
        persistenceId = persistenceId,
        emptyState = ReactivestockState.initial,
...
/**
  ...
  */
case class ReactivestockState(message: String, timestamp: String) {
  def applyCommand(cmd: ReactivestockCommand): ReplyEffect[ReactivestockEvent, ReactivestockState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: ReactivestockEvent): ReactivestockState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[ReactivestockEvent, ReactivestockState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[ReactivestockEvent, ReactivestockState] =
    Effect
...
object ReactivestockState {

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  def initial: ReactivestockState = ReactivestockState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[ReactivestockCommand]("ReactivestockAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[ReactivestockState] = Json.format
}

/**
  * This interface defines all the events that the ReactivestockAggregate supports.
  */
sealed trait ReactivestockEvent extends AggregateEvent[ReactivestockEvent] {
  def aggregateTag: AggregateEventTag[ReactivestockEvent] = ReactivestockEvent.Tag
}

object ReactivestockEvent {
  val Tag: AggregateEventTag[ReactivestockEvent] = AggregateEventTag[ReactivestockEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends ReactivestockEvent
...
/**
  ...
  */
trait ReactivestockCommandSerializable

/**
  * This interface defines all the commands that the ReactivestockAggregate supports.
  */
sealed trait ReactivestockCommand
    extends ReactivestockCommandSerializable
...
/**
  ...
  * application loader.
  */
object ReactivestockSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    // state and events can use play-json, but commands should use jackson because of ActorRef[T] (see application.conf)
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[ReactivestockState],
