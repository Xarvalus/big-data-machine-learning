...
import xarvalus.reactive.stock.api
...
/**
  * Implementation of the ReactivestockService.
  */
class ReactivestockServiceImpl(
  clusterSharding: ClusterSharding,
  persistentEntityRegistry: PersistentEntityRegistry
)(implicit ec: ExecutionContext)
  extends ReactivestockService {

  /**
    * Looks up the entity for the given ID.
    */
  private def entityRef(id: String): EntityRef[ReactivestockCommand] =
    clusterSharding.entityRefFor(ReactivestockState.typeKey, id)
  ...
  override def greetingsTopic(): Topic[api.GreetingMessageChanged] =
    TopicProducer.singleStreamWithOffset { fromOffset =>
      persistentEntityRegistry
        .eventStream(ReactivestockEvent.Tag, fromOffset)
        .map(ev => (convertEvent(ev), ev.offset))
    }

  private def convertEvent(
    helloEvent: EventStreamElement[ReactivestockEvent]
