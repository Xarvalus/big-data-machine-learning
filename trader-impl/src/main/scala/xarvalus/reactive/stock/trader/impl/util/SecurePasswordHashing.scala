...
object SecurePasswordHashing {
  ...
  def hashPassword(password: String, salt: Array[Byte], nrOfIterations: Int): String = {
    ...
    val salt64 = new String(toBase64(salt))
    val hash64 = new String(toBase64(hash))
  ...
  def validatePassword(password: String, hashedPassword: String): Boolean = {
    ...
    val hash = fromBase64(hashParts(1))
    val salt = fromBase64(hashParts(2))

    if (hash.isLeft || salt.isLeft) return false

    // TODO: remove deprecated api calls
  ...
  private def toBase64(bytes: Array[Byte]) =
    Base64.getEncoder.encode(bytes)

  private def fromBase64(str: String) = {
    Base64.getDecoder.decode(str) match {
      case x: Array[Byte] => Right(x)
      case _ => Left(None)
    }
  }
}
